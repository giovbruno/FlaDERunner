...
class LC:
    ...
    def prewhitening(self, nurange=[1., 100.], npeaks=100, \
                upper_freq=3., plots=False, verbose=False):
        '''
        Remove sinusoidal signals from light curve.

        Parameters
        ----------
        upper_freq: remove only frequencies below this value x main detected
        frequency. Alternative to nurange
        '''
        ...
        if upper_freq is not None:
            # Loest and highest possible frequencies are set
        ...
        if plots:
            ...
            plt.xlabel(r'Frequency $\mu$Hz', fontsize=14)
